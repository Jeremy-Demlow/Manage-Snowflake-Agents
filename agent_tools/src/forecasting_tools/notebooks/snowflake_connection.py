...
class ConnectionConfig(BaseModel):
    ...
    create_db_if_missing: bool = Field(
        True, description="Create database/schema if they don't exist"
    )
    ...
    def from_env(cls) -> ConnectionConfig:
        ...
        if not any(
            k in config for k in ["password", "private_key_path", "authenticator"]
        ):
            raise ConfigurationError(
                "No authentication method provided via environment variables"
            )
    ...
    def from_snow_cli(cls, connection_name: Optional[str] = None) -> ConnectionConfig:
        ...
        if not snow_config:
            if connection_name:
                raise ConfigurationError(
                    f"Snow CLI connection '{connection_name}' not found"
                )
            else:
                raise ConfigurationError(
                    "No Snow CLI connections found or default connection not set"
                )
        ...
        if "account" not in config:
            raise ConfigurationError(
                "Missing required field 'account' in Snow CLI connection"
            )
        if "user" not in config:
            raise ConfigurationError(
                "Missing required field 'user' in Snow CLI connection"
            )

        return cls(**config)

    @classmethod
    def from_env_or_snow_cli(
        cls, connection_name: Optional[str] = None
    ) -> ConnectionConfig:
...
class SnowflakeConnection:
                ...
                try:
                    ...
                    if config.create_db_if_missing:
                        ...
                        try:
                            ...
                            raise ConnectionError(
                                f"Database/schema not found and create_db_if_missing=False: {use_error}"
                            )
    ...
    def list_tables(
        self, pattern: Optional[str] = None, schema: Optional[str] = None
    ) -> DataFrame:
    ...
    def get_ddl(self, object_name: str, object_type: str = "TABLE") -> str:
        ...
        try:
            result = self.fetch(
                f"SELECT GET_DDL('{object_type}', '{object_name}') as ddl"
            )
            return result[0]["DDL"] if result else ""
        except Exception as e:
            raise ConnectionError(
                f"Failed to get DDL for {object_type} {object_name}: {str(e)}"
            )
